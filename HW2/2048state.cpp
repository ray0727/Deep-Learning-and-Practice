...
public:
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				for(int i=0; i<num_empty; i++)
				{
					...
					tmp2.popup(1,i);
					tmp4.popup(2,i);
					estimate_value += (1.0/num_empty)*0.9*estimate(tmp2) + (1.0/num_empty)*0.1*estimate(tmp4);
				}
	 ...
	 */
	void update_episode(std::vector<board>& s, std::vector<state>& s1, float alpha = 0.1) const {
		// TODO
		float exact = 0;
		float error;
		for(s.pop_back(); s.size(); s.pop_back())
		{
			...
			debug << "update error = " << error << " for after state" << std::endl << after_s1.after_state();
...
int main(int argc, const char* argv[]) {
	...
	// tdl.save("/home/ray/DL_and_practice/HW2");
	// score_file.close();
	return 0;
}
